                ...
                if(this.type.equals("QueryUrl")) {
                    if (serp.clicked.contains(upperUrl)) {
                        if(queryId.containsKey(serp.query) && urlId.containsKey(link)) {
                            context.write(new Text(Config.PROB_UPPER + Config.DELIMER + queryId.get(serp.query) + Config.DELIMER + urlId.get(link)),
                                    new Text("1"));
                        }
                    } else {
                        if(queryId.containsKey(serp.query) && urlId.containsKey(link)) {
                    ...
                    if (serp.clicked.contains(upperUrl)) {
                        if(urlId.containsKey(link)) {
                            context.write(new Text(Config.PROB_UPPER + Config.DELIMER + urlId.get(link)),
                                    new Text("1"));
                        }
                    } else {
                        if(urlId.containsKey(link)) {
