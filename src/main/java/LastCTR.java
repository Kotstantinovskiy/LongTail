...
public class LastCTR {
    ...
    LastCTR(HashSet<String> trainSet,
    ...
    public void map(Mapper.Context context, SERP serp) throws IOException, InterruptedException {
        int idx;
        String lastUrl = serp.clicked.get(serp.clicked.size()-1);

        if (trainSet.contains(lastUrl) || testSet.contains(lastUrl)) {
                ...
                if (trainSet.contains(url) || testSet.contains(url)) {
                    ...
                    if (type.equals("QueryUrl")) {
                        if(queryId.containsKey(serp.query) && urlId.containsKey(url)) {
                            context.write(new Text(Config.LAST_CTR + Config.DELIMER + queryId.get(serp.query) + Config.DELIMER + urlId.get(url)), new Text(String.valueOf(idx)));
                        }
                    } else {
                        if(urlId.containsKey(url)) {
                            context.write(new Text(Config.LAST_CTR + Config.DELIMER + urlId.get(url)),
