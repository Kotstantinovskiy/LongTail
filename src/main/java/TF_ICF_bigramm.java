...
public class TF_ICF_bigramm extends Configured implements Tool {

    public static class TFICFMapper extends Mapper<LongWritable, Text, Text, DoubleWritable> {
        ...
        protected void setup(Mapper.Context context) throws IOException {
            Path docs_df = new Path(Config.DOCS_ALL_ICF_BI);
            ...
            while (word_df != null && !word_df.equals("")) {
                cf.put(word_df.split("\t")[0], Double.valueOf(word_df.split("\t")[1]));
        ...
        public void map(LongWritable key, Text line, Context context) throws IOException, InterruptedException {
            ...
            if (parts.length == 3) {
                ...
                if (idxQueries != null) {
                    ...
                    for (int i=1; i < textTokens.size(); i++) {
                        String word = textTokens.get(i-1) + " " + textTokens.get(i);
                        ...
                        if (w.length() != 0) {
                            if (cf.containsKey(w)) {
                                for (int idx : idxQueries) {
                                    ...
                                    if (query != null) {
                                        if (query.length() != 0 && query.split(" ").length > 1) {
                                            for(int i=1; i < query.split(" ").length; i++) {
                                                String ww = query.split(" ")[i-1] + " " + query.split(" ")[i];
                                                if (w.equals(ww)) {
                                                    double idf = Math.log(Config.LEN_ALL / cf.get(w));
                                                    if (idf > 0) {
                    ...
                    for (int i=1; i < titleTokens.size(); i++) {
                        String word = titleTokens.get(i-1) + " " + titleTokens.get(i);
    ...
    public static class TFICFReducer extends Reducer<Text, DoubleWritable, Text, DoubleWritable> {
        @Override
    ...
    public static void main(String[] args) throws Exception{
        int exitCode = ToolRunner.run(new TF_ICF_bigramm(), args);
    ...
    public int run(String[] args) throws Exception {
        ...
        job.setJobName("TF_ICF_bigramm");

        job.setJarByClass(TF_ICF_bigramm.class);
