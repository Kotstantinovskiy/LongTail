...
public class OnlyCTR {
    ...
    OnlyCTR(HashSet<String> trainSet,
            ...
            if (trainSet.contains(url) || testSet.contains(url)) {
                ...
                if (type.equals("QueryUrl")) {
                    if(queryId.containsKey(serp.query) && urlId.containsKey(url)) {
                        context.write(new Text(Config.ONLY_CTR + Config.DELIMER + queryId.get(serp.query) + Config.DELIMER + urlId.get(url)),
                                new Text(String.valueOf(idx)));
                    }
                } else {
                    if(urlId.containsKey(url)) {
                        context.write(new Text(Config.ONLY_CTR + Config.DELIMER + urlId.get(url)), new Text(String.valueOf(idx)));
                    }
                }
            }
        }
